...
export default function OrdersPage() {
  ...
  // Check sessionStorage for flash messages (e.g., after deleting an order)
  useEffect(() => {
    const stored = sessionStorage.getItem("ordersFlashMessage");
    if (stored) {
      const { message, type } = JSON.parse(stored);
      setAlert({ type: type || "success", alertMessage: message });
      sessionStorage.removeItem("ordersFlashMessage");
    }
  }, []);

  const getImageUrl = (thumbnailId: string | undefined): string => {
  ...
  const handleAcknowledgeCancellation = async (orderId: string) => {
                  ...
                  <div className="flex-1">
                    ...
                    {order.cancellationReason && (
                      <div className="mt-2 rounded bg-orange-100 p-2 border border-orange-200">
                        ...
                        <p className="text-sm text-orange-900 wrap-break-word">
                          {order.cancellationReason}
                        </p>
                      </div>
                    )}
                    ...
                    {order.items && order.items.length > 0 && (
                      <div className="flex gap-1.5">
                        {order.items.slice(0, 3).map((item, index) => (
                          <div
                            key={index}
                            className="relative h-12 w-12 shrink-0 overflow-hidden rounded border border-gray-200"
                          >
                            <Image
                              src={
                        ...
                        {order.items.length > 3 && (
                          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded border border-gray-200 bg-gray-100 text-xs font-medium text-gray-600">
                            +{order.items.length - 3}
                          </div>
                        )}
                      </div>
