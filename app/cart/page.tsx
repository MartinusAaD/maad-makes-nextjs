...
interface CartFormData extends Record<string, unknown> {
  firstName: string;
  lastName: string;
  phone: string;
  email: string;
...
export default function CartPage() {
  ...
  const validationRules = () => ({
    ...
    postalCode: {
      ...
      pattern: { value: "^[0-9]{4}$", message: "Postal code must be 4 digits" },
    },
    city: {
  ...
  const {
    ...
    hasError,
    isSubmitting,
  } = useFormValidation<CartFormData>(
    {
      firstName: "",
      lastName: "",
      phone: "",
  ...
  const confirmPlaceOrder = async () => {
    ...
    try {
      ...
      trackPurchase({ ...orderData, orderNumber: result.orderNumber });
      clearCart();
  ...
  const getItemImage = (thumbnailId: string | undefined) => {
